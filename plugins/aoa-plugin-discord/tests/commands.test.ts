...
const mockAoaFetch = vi.fn().mockResolvedValue({ ok: true, status: 200, headers: new Headers(), text: () => Promise.resolve("") });
vi.mock("../src/aoa-fetch.js", () => ({
  aoaFetch: (...args: unknown[]) => mockAoaFetch(...args),
}));

beforeEach(() => {
  mockAoaFetch.mockReset().mockResolvedValue({ ok: true, status: 200, headers: new Headers(), text: () => Promise.resolve("") });
});
...
describe("/clip status", () => {
  it("returns agent and issue data with active work and completions", async () => {
    ...
    expect(embed.title).toBe("AoA Status");
...
describe("/clip approve", () => {
  ...
  it("calls approval API with correct URL", async () => {
    ...
    expect(mockAoaFetch).toHaveBeenCalledWith(
  ...
  it("returns error when API returns non-OK status (Bug 1 regression)", async () => {
    mockAoaFetch.mockResolvedValue({
      ok: false,
      status: 422,
      headers: new Headers(),
...
describe("button clicks", () => {
  ...
  it("shows failure when approve API call fails", async () => {
    mockAoaFetch.mockRejectedValueOnce(new Error("All resolved IPs for localhost are in private/reserved ranges"));
    const ctx = makeCtx();
    const result = await handleInteraction(
      ctx,
      {
  ...
  it("shows failure when reject API call fails", async () => {
    mockAoaFetch.mockRejectedValueOnce(new Error("Network error"));
    const ctx = makeCtx();
    const result = await handleInteraction(
      ctx,
      {
...
describe("issue_assign button", () => {
  ...
  it("returns a clearer message when the Discord user is not linked to a board user", async () => {
    ...
    expect(result.type).toBe(4);
    expect(result.data.content).toContain("not linked to a AoA board user");
  });
