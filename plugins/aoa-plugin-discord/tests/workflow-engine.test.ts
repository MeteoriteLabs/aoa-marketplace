...
const mockAoaFetch = vi.fn().mockResolvedValue({
  ok: true,
  status: 200,
...
beforeEach(() => {
  mockAoaFetch.mockReset().mockResolvedValue({
    ok: true,
    status: 200,
    json: () => Promise.resolve({ id: "issue-1", title: "Test Issue" }),
    text: () => Promise.resolve("ok"),
...
describe("runWorkflow — step types", () => {
  ...
  it("http_request makes fetch call via ctx.http.fetch", async () => {
    const ctx = makeCtx();
    // http_request must use ctx.http.fetch (not aoaFetch) to preserve private-IP restriction
    ...
    // aoaFetch must NOT be used for user-controlled URLs
    expect(mockAoaFetch).not.toHaveBeenCalledWith("https://example.com/api", expect.anything());
  });
...
describe("runWorkflow — multi-step and errors", () => {
  it("stops on step failure", async () => {
    const ctx = makeCtx();
    mockAoaFetch.mockResolvedValueOnce({
      ok: false, status: 404,
