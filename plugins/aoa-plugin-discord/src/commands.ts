...
import { aoaFetch } from "./aoa-fetch.js";
...
export interface CommandContext {
  ...
  token: string;
  /** Optional AoA board API key — attached to AoA API calls that
...
export const SLASH_COMMANDS = [
  {
    name: "clip",
    description: "Manage your AoA instance from Discord",
    options: [
      {
        name: "status",
      ...
      {
        name: "connect",
        description: "Link this channel to an AoA company",
        type: 1,
        options: [
          {
            name: "company",
      ...
      },
      {
        ...
        description: "Map current Discord channel to an AoA project",
        type: 1,
        options: [
          {
            name: "project",
...
async function handleButtonClick(
  ...
  const apiKey = cmdCtx?.aoaBoardApiKey ?? "";
  ...
  if (customId.startsWith("approval_approve_")) {
    ...
    try {
      const resp = await withRetry(async () => {
        const r = await aoaFetch(`${base}/api/approvals/${approvalId}/approve`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  if (customId.startsWith("approval_reject_")) {
    ...
    try {
      const resp = await withRetry(async () => {
        const r = await aoaFetch(`${base}/api/approvals/${approvalId}/reject`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ decidedByUserId: `discord:${actor}` }),
        }, apiKey);
  ...
  if (customId.startsWith("issue_reopen_")) {
    ...
    try {
      ...
      return {
        type: 7,
        data: {
          embeds: [{ title: "Reopen Failed", description: `Could not reopen — ${err instanceof Error ? err.message : String(err)}`, color: COLORS.RED, footer: { text: "AoA" }, timestamp: new Date().toISOString() }],
          components: [],
        },
      };
    }
    return {
      type: 7,
      data: {
        embeds: [{ title: "Issue Reopened", description: `Reopened by **${actor}**`, color: COLORS.YELLOW, footer: { text: "AoA" }, timestamp: new Date().toISOString() }],
        components: [],
      },
    };
  }
  ...
  if (customId.startsWith("issue_assign_")) {
    ...
    try {
      ...
      const friendlyMessage = rawMessage.includes("Assignee user not found")
        ? "your Discord user is not linked to a AoA board user"
        : rawMessage;
...
async function handleCommandsImport(
  ...
  return respondToInteraction({
    ...
    embeds: [{
      ...
      footer: { text: "AoA Workflow Commands" },
      timestamp: new Date().toISOString(),
    }],
  });
}
