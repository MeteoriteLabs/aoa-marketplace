  ...
  instanceConfigSchema: {
    ...
    properties: {
      ...
      aoaBoardApiKeyRef: {
        type: "string",
        format: "secret-ref",
        title: "AoA Board API Key (secret reference)",
        description:
          "Optional. Secret UUID for a AoA board API key. Required when AoA is deployed in `authenticated` mode so that plugin-originated calls (approve/reject buttons, workflow steps, inbound reply routing) can satisfy server-side board-auth checks. Create a board API key in Settings → API Keys, store it as a secret, then paste the secret UUID here. Leave blank for `local_trusted` deployments.",
        default: DEFAULT_CONFIG.aoaBoardApiKeyRef,
      },
      defaultGuildId: {
        type: "string",
      ...
      companyChannels: {
        ...
        description:
          "Route notifications per AoA company. Keys are company UUIDs, values are Discord channel IDs. Applied to every event type that does not have a more specific map. Falls through to the default/global channel when a company is not listed.",
      ...
      aoaBaseUrl: {
        type: "string",
        title: "AoA Base URL",
        description: "Base URL for AoA dashboard links and API calls",
      ...
      enableInbound: {
        ...
        description:
          "Route Discord replies to bot notifications back to AoA as issue comments or escalation responses.",
