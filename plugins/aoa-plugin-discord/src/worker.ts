...
type DiscordConfig = {
  ...
  /**
   * Per-company channel overrides. Keys are AoA company UUIDs; values are
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    const aoaBoardApiKey = config.aoaBoardApiKeyRef
      ? await ctx.secrets.resolve(config.aoaBoardApiKeyRef)
      : "";
      ...
      for (const company of companies) {
        ...
        try {
          ...
          const companyLabel = company.name ? ` — ${company.name}` : "";
            ...
            const footerText = `AoA • ${completedToday.length} completed, ${blocked.length} blocked, ${inProgress.length} in progress`;
