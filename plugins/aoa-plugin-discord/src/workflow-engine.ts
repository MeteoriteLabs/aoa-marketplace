...
async function execFetchIssue(
  ...
  try {
    const resp = await aoaFetch(`${baseUrl}/api/issues/${issueId}`, {
      method: "GET",
...
async function execHttpRequest(
  ...
  try {
    // Use ctx.http.fetch (NOT aoaFetch) for user-configured URLs to
...
async function execCreateIssue(
  ...
  try {
    const resp = await aoaFetch(`${baseUrl}/api/companies/${companyId}/issues`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export interface WorkflowRunOptions {
  ...
  companyId: string;
  baseUrl: string;
  /** AoA board API key. Empty string disables Authorization header
...
export async function runWorkflow(opts: WorkflowRunOptions): Promise<{
  ...
  const { ctx, token, channelId, companyId, baseUrl, aoaBoardApiKey, workflow, args } = opts;
  ...
  for (let i = startStep; i < workflow.steps.length; i++) {
    ...
    switch (step.type) {
      case "fetch_issue":
        result = await execFetchIssue(ctx, step, wfCtx, baseUrl, aoaBoardApiKey);
        break;
      case "invoke_agent":
      ...
      case "send_message":
        ...
        break;
      case "create_issue":
        result = await execCreateIssue(ctx, step, wfCtx, companyId, baseUrl, aoaBoardApiKey);
        break;
      case "wait_approval":
