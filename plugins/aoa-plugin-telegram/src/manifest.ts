...
const manifest: PaperclipPluginManifestV1 = {
  ...
  instanceConfigSchema: {
    ...
    properties: {
      ...
      aoaBaseUrl: {
        type: "string",
        title: "AoA API URL (internal)",
        description:
          "Internal URL of the AoA API server. Used for API calls (approvals, comments). Keep as localhost for same-server deployments.",
        default: DEFAULT_CONFIG.aoaBaseUrl,
      },
      aoaBoardApiTokenRef: {
        type: "string",
        format: "secret-ref",
        title: "AoA Board API Token (secret reference)",
        description:
          "Secret UUID for an AoA board API token. Used by Telegram approval buttons and /approve commands to resolve approvals as a board actor.",
        default: DEFAULT_CONFIG.aoaBoardApiTokenRef,
      },
      aoaPublicUrl: {
        type: "string",
        title: "AoA Public URL",
        description:
          "Public URL for issue links in Telegram messages (e.g. https://aoa.example.com). Falls back to API URL if empty.",
        default: DEFAULT_CONFIG.aoaPublicUrl,
      },
      ...
      topicRouting: {
        ...
        description:
          "Map Telegram forum topics to AoA projects. Requires the bot to be in a group with forum topics enabled.",
      ...
      onlyNotifyIfAssignedTo: {
        ...
        description:
          "Optional. Paste your AoA user ID here to restrict assignment notifications to items newly assigned to you. Leave empty to notify on every assignment change. To find your user ID: sign in to AoA in a browser and visit /api/cli-auth/me — copy the `userId` field (a string like `mrvhBEpPds85TGeEjlAHviP0VdOHgymm`).",
      ...
      enableCommands: {
        ...
        description:
          "Allow users to interact with AoA via Telegram bot commands (/status, /issues, /agents). If this is enabled, consider configuring the Telegram user/chat allowlists below.",
      ...
      enableInbound: {
        ...
        description:
          "Route Telegram messages to AoA issue comments. Messages sent in reply to a notification get attached to that issue. If this is enabled, consider configuring the Telegram user/chat allowlists below.",
