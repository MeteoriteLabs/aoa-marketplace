...
function normalizeBoardAccessState(value: unknown): TelegramBoardAccessState {
  ...
  return {
    aoaBoardApiTokenRef: asNonEmptyString(record.aoaBoardApiTokenRef),
...
async function persistBoardAccessState(
  ...
  return {
    ...nextState,
    configured: Boolean(nextState.aoaBoardApiTokenRef),
  };
}
...
function getBoardAccessRegistration(
  ...
  return {
    ...state,
    configured: Boolean(state.aoaBoardApiTokenRef),
  };
}
...
async function resolveBoardApiToken(
  ...
  if (
    boardAccessState.aoaBoardApiTokenRef &&
    (!companyId || !boardAccessState.companyId || boardAccessState.companyId === companyId)
  ) {
    candidates.push({
      source: "board-access",
      ref: boardAccessState.aoaBoardApiTokenRef,
    });
  }

  if (config.aoaBoardApiTokenRef) {
    candidates.push({
      source: "config",
      ref: config.aoaBoardApiTokenRef,
    });
  }
 ...
 *
 * AoA's core can emit duplicate `issue.updated` plugin events for a
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    const baseUrl = config.aoaBaseUrl || "http://localhost:3100";
    const publicUrl = config.aoaPublicUrl || baseUrl;
    ...
    ctx.actions.register("board-access.update", async (params) => {
      ...
      const aoaBoardApiTokenRef = asNonEmptyString(record.aoaBoardApiTokenRef);
