import { definePlugin, runWorker } from "@armyofagents/plugin-sdk";
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    // ---------------------------------------------------------------
    // AoA event: issue status changed -> sync to GitHub
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // AoA event: comment added -> bridge to GitHub
    // ---------------------------------------------------------------
    ctx.events.on("issue.comment_added", async (event) => {
      ...
      const authorName =
        (event.payload?.authorName as string) || "AoA user";
    ...
    ctx.webhooks?.register(WEBHOOK_KEYS.github, async (delivery) => {
      ...
      if (event === "issue_comment" && action === "created") {
        ...
        // Skip comments bridged from AoA (prevent echo loop)
        if (commentBody?.includes("[synced from AoA]")) return;
